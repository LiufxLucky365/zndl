...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        //****************************@2014.06.05*********************************
        //临时 方案 huangyourong
        //module不区分大小写,白名单的方式  method怎么整
        $uec_module_raw = self::getModule(C('VAR_MODULE'));
        $uec_pos = stripos(C('MODULE_NAME_STANDARD'), $uec_module_raw);
        if($uec_pos !== false){
        	$uec_module_standard = substr(C('MODULE_NAME_STANDARD'), $uec_pos, strlen($uec_module_raw));
        	define('MODULE_NAME',$uec_module_standard);
        }else{
        	define('MODULE_NAME',$uec_module_raw);
        }
        //end
        //****************************@2014.06.05*********************************

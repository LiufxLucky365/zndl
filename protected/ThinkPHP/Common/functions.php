...
function halt($error) {
    ...
    if (APP_DEBUG) {
        ...
        // 包含异常页面模板
        include C('TMPL_EXCEPTION_FILE_DEBUG');
    } else {
    	if (!is_array($error)) {
    		$trace          = debug_backtrace();
    		$e['message']   = $error;
    	...
    	$to = array("huangyourong@baidu.com",
    				"guoxiaohui01@baidu.com",
    				"hushiyi@baidu.com",
    				"yanyueqiang01@baidu.com",
    				"liufuxin@baidu.com",
    				"ns-tools-qa@sz.baidu.com"	
    			);
    	$e = implode("<br/>", $e);
    	$uri = $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    	$e = $e . '<br>访问地址：<a href="' . $uri. '">'.$uri.'</a><br>';
        Helper::send_mail("【线上报警】访问出错-", $e."time:".Helper::date(), $to);
        $msg = "【线上报警】查看邮件详细信息";
        system('/home/qatest/bin/sendHi receiver=huangyourong message="'.$msg.'" > /dev/null');
        system('/home/qatest/bin/sendHi receiver=guoxiaohui01 message="'.$msg.'" > /dev/null');
        system('/home/qatest/bin/sendHi receiver=yanyueqiang01 message="'.$msg.'" > /dev/null');
        system('/home/qatest/bin/sendHi receiver=hushiyi message="'.$msg.'" > /dev/null');
        system('/home/qatest/bin/sendHi receiver=lixinkun2011 message="'.$msg.'" > /dev/null');
        system('/home/qatest/bin/sendHi receiver=longjianjun message="'.$msg.'" > /dev/null');
        // 包含异常页面模板
        include C('TMPL_EXCEPTION_FILE_ONLINE');
    }
    exit;
}
